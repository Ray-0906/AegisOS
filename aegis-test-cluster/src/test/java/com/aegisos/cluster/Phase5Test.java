...
class Phase5Test {
    ...
    void submitRunAndCollectResult() throws Exception {
        try (ClusterHarness harness = new ClusterHarness()) {
            ...
            JobHandle handle = submitter.api().getProcessManager().submit(new PrimeCounter(10_000));
    ...
    void schedulesManyJobs() throws Exception {
        try (ClusterHarness harness = new ClusterHarness()) {
            ...
            for (int i = 0; i < jobs; i++) {
                handles.add(submitter.api().getProcessManager().submit(new PrimeCounter(1_000)));
            }
