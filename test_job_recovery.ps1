...
Write-Host "Pausing Node $workerNode to trigger migration (35 seconds)..."
$workerProcess = Get-Process -Id $workerPid
[PM]::NtSuspendProcess($workerProcess.Handle)

Start-Sleep -Seconds 35

Write-Host "Resuming Node $workerNode to test fencing..."
[PM]::NtResumeProcess($workerProcess.Handle)
...
Write-Host "Checking for fencing in logs..."
$logContent = Get-Content "data/node$workerNode.log"
if ($logContent -match "Fencing rejected update") {
    Write-Host "SUCCESS: Job fencing worked! The delayed node's job updates were rejected."
} else {
    Write-Host "FAILURE: No fencing log found."
    Write-Host "CLI Output:"
    Get-Content data/job.log
    exit 1
}

Write-Host "Job recovery test done."
